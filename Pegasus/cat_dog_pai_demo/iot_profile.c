...
static char *MakeProfileReport(WeChatProfile *payload)
{
    ...
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionMotor_cat,
                            payload->reportAction.motor_catActionStatus);
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionMotor_dog,
                            payload->reportAction.motor_dogActionStatus);
    ret = cJSON_PrintUnformatted(root);
    cJSON_Delete(state);
...
#define CN_PROFILE_TOPICFMT_TOPIC            "$shadow/operation/2JKD3N8XCO/Hi3861_mqtt"
