...
void UartSendRead(int fd, refuseClassification refuseType)
{
    ...
    char *data3 = "0x04";
    char *data4 = "0x08";
    // unsigned char writeBuffer1[buf_size] = {1, 1, 1, 1};
    // unsigned char writeBuffer2[buf_size] = {2, 2, 2, 2};
    // unsigned char writeBuffer3[buf_size] = {3, 2, 3, 3};
    // unsigned char writeBuffer4[buf_size] = {4, 4, 4, 4};
    ...
    switch (refuseType) {
        case Fistecls:
            //HisignallingMsgSend(fd, writeBuffer1, sizeof(writeBuffer2)/sizeof(writeBuffer2[0]));
            UartSend(fd, (char *)data1, strlen(data1));
            printf("send classes status:buffalo\r\n");
            break;
        case Secondcls:
            //HisignallingMsgSend(fd, writeBuffer2, sizeof(writeBuffer3)/sizeof(writeBuffer3[0]));
            UartSend(fd, (char *)data2,strlen(data2));
            printf("send classes status:elephant\r\n");
            break;
        case Thirdcls:
            //HisignallingMsgSend(fd, writeBuffer3, sizeof(writeBuffer4)/sizeof(writeBuffer4[0]));
            UartSend(fd, (char *)data3, strlen(data3));
            printf("send classes status:rhino\r\n");
            break;
        case Forthcls:
            //HisignallingMsgSend(fd, writeBuffer4, sizeof(writeBuffer5)/sizeof(writeBuffer5[0]));
            UartSend(fd, (char *)data4, strlen(data4));
            printf("send classes status:zebra\r\n");
            break;
            /*
        case YesGesture:
