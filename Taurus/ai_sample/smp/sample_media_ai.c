...
extern SampleStoreInfo g_sampleStoreInfo;
...
static VENC_CHN VencChn[3] = {0, 1, 2};
static VPSS_CHN VpssChn[3] = {0, 1, 2};
...
extern uart_flag1; 
extern int class_get;
extern int g_s32Quit ;

//extern int uart_flag1=0;
...
{
    ViCfgInit1(&g_aicMediaInfo.viCfg);
...
// void *uart_handle(HI_VOID)
// {
//     SAMPLE_PRT("---------------uart_handle-------------\n");
//     Uart_receive();
// }

void *g_s32Quit_handle()
{
    Pause();
    g_s32Quit=1;
}
...
HI_S32 SAMPLE_MEDIA_HAND_CLASSIFY(HI_VOID)
{
    ...
    // s32Ret = pthread_create(&g_aiProcessThread, NULL, uart_handle, NULL);
    ...
    sleep(5);
    // Pause();
    if((class_get>=1)&&(class_get<=4))    //识别到了目标 10s
        j=50;
    else                        //没识别到目标   5s
        j=5;
...
EXIT:
    ...
    if((class_get>=1)&&(class_get<=4))
    {
        ...
        pthread_detach(id);
        pthread_create(&id2, NULL, g_s32Quit_handle, NULL);
        while (!g_s32Quit)
        {
            nanosleep(&ts, NULL);
            EventLoop(s32MainFd);
